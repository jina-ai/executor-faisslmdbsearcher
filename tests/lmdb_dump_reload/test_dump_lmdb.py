...
def test_dump_reload(tmpdir, nr_docs, emb_size, shards):
    ...
    with Flow.load_config(storage_flow_yml) as flow_storage:
        with Flow.load_config(query_flow_yml) as flow_query:
            ...
            result_docs = flow_query.post(on='/search', inputs=docs)
            assert len(result_docs[0].matches) == 0
            ...
            logger.warning(f'doing rolling update...')
            flow_query.rolling_update(
                deployment_name='indexer_query', uses_with={'dump_path': dump_path}
            )

            result_docs = flow_query.post(
                on='/search',
                inputs=docs,
                parameters={'limit': top_k},
            )
            assert len(result_docs[0].matches) == top_k
            # TODO score is not deterministic
            for i in range(len(result_docs[0].matches) - 1):
                assert (
                    result_docs[0].matches[i].scores[METRIC].value
                    <= result_docs[0].matches[i + 1].scores[METRIC].value
                )
